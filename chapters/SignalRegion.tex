...
%\indent However, the $\ttbar$ differential cross section as a function of $m^{t\tbar}$, shown in Figure \ref{fig:ttbar:mtt}, falls exponentially.  $m^{t\tbar}$ is directly related to the amount of back-to-back boost between the two tops and the $\pt$ of each top.  .  Thereby the neutrino is able to gain enough momentum to pass the $\met > 250 \gev$ requirement while the total amount of $m^{\ttbar}$ remains low.


%After the $\met > 250 \gev$ requirement the neutrino must a certain $\pt$ to pass the $\met$ requirement. In order to meet the $\pt$ requirement, the neutrino perfermost $\ttbar$ events have a low $m^{t\tbar}$ but the neutrino receives a large fraction of the top $\pt$ instead of having a high $m^{t\tbar}$ and the neutrino receiving a small fraction of the top $\pt$.   \\
